...
# nfeature = 8
ifeature = 1
mascon_dates_daily = pd.date_range(start='2004-12-01',end='2013-01-01',freq='D')
...
for In_NAME in In_NAMEs:
    print(In_NAME)
    In_CSV = os.path.join(dir_csv, 'EWH-{}.csv'.format(In_NAME))  # unit: cm

    # for ifeature in range(nfeature):
    OUT_CSV = os.path.join(dir_csv, 'dS-{}-{}.csv'.format(In_NAME, ifeature))
