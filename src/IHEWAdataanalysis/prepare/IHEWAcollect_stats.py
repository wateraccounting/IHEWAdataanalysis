            ...
            for date in dates:
                ...
                if date_month == date.month:
                    date_out = pd.Timestamp(
                        '{:04d}-{:02d}-{}'.format(date_year, date_month, '01'))
                    ...
                    if date == date_e:
                        ...
                        if tif[var]['name'].format(dtime=date_out).split('_')[
                            2] == 'mm.m' or \
                                tif[var]['name'].format(dtime=date_out).split('_')[
                                    2] == 'mm':
                            file_tif_monthly = os.path.join(dir_tmp,
                                                            tif[var]['output'].format(
                                                                dtime=date_out))
                            print(file_tif_monthly)

                            shutil.copyfile(file_tif, file_tif_monthly)

                            ds_stats = zonal_stats(file_shp, file_tif,
                                                   stats=' '.join(stats_names))
                            fp.write(date_str)
                            for i in range(ds_shp_layer_fea_n):
                            ...
                            Save_as_tiff(name=file_tif_monthly, data=data,
                                         geo=geo_trans, projection="WGS84")

                            ds_stats = zonal_stats(file_shp, file_tif_monthly,
                                                   stats=' '.join(stats_names))
                            fp.write(date_str)
                            for i in range(ds_shp_layer_fea_n):
                                for key in stats_names:
                    ...
                    if tif[var]['name'].format(dtime=date_out).split('_')[
                        ...
                        data = np.where(data == 0.0, ds_ndv, data)
                        Save_as_tiff(name=file_tif_monthly, data=data, geo=geo_trans,
                                     projection="WGS84")
